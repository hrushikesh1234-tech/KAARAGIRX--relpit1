...
const PublicDealerProfile = () => {
  ...
  const fetchMaterials = async () => {
    try {
      const response = await fetch(`/api/materials/dealer/${id}`, { credentials: 'include' });
      if (response.ok) {
        const data = await response.json();
        setMaterials(data);
      }
  ...
  const portfolioItems: PortfolioItem[] = materials.map(m => ({
    id: String(m.id),
    title: m.name,
    thumbnail: m.image || m.images?.[0] || '/placeholder.jpg',
  ...
  const profileData = {
    username: dealer?.companyName || 'Material Dealer',
    displayName: dealer?.companyName || 'Material Dealer',
    bio: dealer?.about || '',
    occupation: 'Material Dealer',
    additionalInfo: dealer?.location || '',
    stats: {
  ...
  const handleFollowClick = async () => {
    if (!user) {
      toast({
        title: "Error",
        description: "Please login to follow dealers",
        variant: "destructive",
      });
      return;
    }
          ...
          <div className="space-y-3">
            ...
            {dealer.yearsInBusiness && (
              <div>
                ...
                <p className="text-white">{dealer.yearsInBusiness} years</p>
              </div>
            )}
            <div>
              <span className="text-gray-400 text-sm">Location:</span>
              <p className="text-white">{dealer.location}</p>
            </div>
            {dealer.phone && (
              <div>
            ...
            {dealer.email && (
              <div>
                ...
                <p className="text-white">{dealer.email}</p>
              </div>
            )}
          </div>
        </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  {selectedReel.price && (
                    <div className="bg-gray-900 p-4 rounded-lg border border-gray-800">
                      ...
                      <p className="text-lg font-medium text-white">₹{selectedReel.price}/{selectedReel.unit}</p>
                    </div>
                  )}
                </div>
